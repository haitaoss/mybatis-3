...
public class DefaultResultSetHandler implements ResultSetHandler {
    ...
    public List<Object> handleResultSets(Statement stmt) throws SQLException {
        ...
        // 如果 rsw(结果集) 不是null 那么必须得有 resultMaps
    ...
    public <E> Cursor<E> handleCursorResultSets(Statement stmt) throws SQLException {
        ErrorContext.instance()
                .activity("handling cursor results")
                .object(mappedStatement.getId());
    ...
    private void validateResultMapsCount(ResultSetWrapper rsw, int resultMapCount) {
        if (rsw != null && resultMapCount < 1) {
            throw new ExecutorException(
                    "A query was run and no Result Maps were found for the Mapped Statement '" + mappedStatement.getId() + "'.  It's likely that neither a Result Type nor a Result Map was specified.");
        }
    }
        ...
        if (rowValue != null) {
            ...
            if (rowValue != null && !hasTypeHandlerForResultObject(rsw, resultMap.getType())) {
                ...
                foundValues = applyNestedResultMappings(
                        rsw, resultMap, metaObject, columnPrefix, combinedKey, true) || foundValues;
                ancestorObjects.remove(resultMapId);
                foundValues = lazyLoader.size() > 0 || foundValues;
                rowValue = foundValues || configuration.isReturnInstanceForEmptyRow() ? rowValue : null;
            }
    ...
    private void linkToParents(ResultSet rs, ResultMapping parentMapping, Object rowValue) throws SQLException {
        CacheKey parentKey = createKeyForMultipleResults(
    ...
    private void addPendingChildRelation(ResultSet rs, MetaObject metaResultObject,
                                         ResultMapping parentMapping) throws SQLException {
        CacheKey cacheKey = createKeyForMultipleResults(
    ...
    private Object createResultObject(ResultSetWrapper rsw, ResultMap resultMap, List<Class<?>> constructorArgTypes,
        ...
        if (hasTypeHandlerForResultObject(rsw, resultType)) {
            ...
            return createByConstructorSignature(
                    rsw, resultMap, columnPrefix, resultType, constructorArgTypes, constructorArgs);
        }
    ...
    private Object createByConstructorSignature(ResultSetWrapper rsw, ResultMap resultMap, String columnPrefix,
        ...
        return applyConstructorAutomapping(rsw, resultMap, columnPrefix, resultType, constructorArgTypes,
                constructorArgs, findConstructorForAutomapping(resultType, rsw).orElseThrow(() -> new ExecutorException(
    ...
    private Optional<Constructor<?>> findConstructorForAutomapping(final Class<?> resultType, ResultSetWrapper rsw) {
        ...
        if (annotated.isPresent()) {
            ...
            return Arrays.stream(constructors)
                    .filter(x -> findUsableConstructorByArgTypes(x, rsw.getJdbcTypes()))
                    .findAny();
        }
    }
    ...
    private Object applyConstructorAutomapping(ResultSetWrapper rsw, ResultMap resultMap, String columnPrefix,
        ...
        if (configuration.isArgNameBasedConstructorAutoMapping()) {
            ...
            foundValues = applyColumnOrderBasedConstructorAutomapping(rsw, constructorArgTypes, constructorArgs,
                    constructor, foundValues
            );
        }
        return foundValues || configuration.isReturnInstanceForEmptyRow() ? objectFactory.create(
                resultType, constructorArgTypes, constructorArgs) : null;
    }

    private boolean applyColumnOrderBasedConstructorAutomapping(ResultSetWrapper rsw,
    ...
    private boolean applyArgNameBasedConstructorAutoMapping(ResultSetWrapper rsw, ResultMap resultMap,
                                                            String columnPrefix, Class<?> resultType,
                                                            List<Class<?>> constructorArgTypes,
                                                            List<Object> constructorArgs, Constructor<?> constructor,
                                                            boolean foundValues) throws SQLException {
            ...
            for (String columnName : rsw.getColumnNames()) {
                if (columnMatchesParam(columnName, paramName, columnPrefix)) {
                    ...
                    if (!autoMappingsCache.containsKey(mapKey)) {
                        MapUtil.computeIfAbsent(constructorAutoMappingColumns, mapKey, k -> new ArrayList<>())
                                .add(columnName);
                    }
    ...
    private boolean columnMatchesParam(String columnName, String paramName, String columnPrefix) {
        ...
        return paramName.equalsIgnoreCase(
    ...
    private Object createPrimitiveResultObject(ResultSetWrapper rsw, ResultMap resultMap,
        ...
        if (!resultMap.getResultMappings()
                .isEmpty()) {
    ...
    private Object getNestedQueryConstructorValue(ResultSet rs, ResultMapping constructorMapping,
        ...
        final Class<?> nestedQueryParameterType = nestedQuery.getParameterMap()
                .getType();
        ...
        if (nestedQueryParameterObject != null) {
            ...
            final CacheKey key = executor.createCacheKey(
    ...
    private Object prepareCompositeKeyParameter(ResultSet rs, ResultMapping resultMapping, Class<?> parameterType,
        ...
        for (ResultMapping innerResultMapping : resultMapping.getComposites()) {
            ...
            final Object propValue = typeHandler.getResult(
                    rs, prependPrefix(innerResultMapping.getColumn(), columnPrefix));
        ...
        for (ResultMapping resultMapping : resultMap.getPropertyResultMappings()) {
            ...
            if (nestedResultMapId != null && resultMapping.getResultSet() == null) {
                try {
                    ...
                    final ResultMap nestedResultMap = getNestedResultMap(
                            rsw.getResultSet(), nestedResultMapId, columnPrefix);
                    ...
                    throw new ExecutorException(
                            "Error getting nested result map values for '" + resultMapping.getProperty() + "'.  Cause: " + e,
                            e
                    );
                }
            }
    ...
    private String getColumnPrefix(String parentPrefix, ResultMapping resultMapping) {
        ...
        return columnPrefixBuilder.length() == 0 ? null : columnPrefixBuilder.toString()
    ...
    private void createRowKeyForUnmappedProperties(ResultMap resultMap, ResultSetWrapper rsw, CacheKey cacheKey,
        ...
        for (String column : unmappedColumnNames) {
            ...
            if (columnPrefix != null && !columnPrefix.isEmpty()) {
                ...
                if (column.toUpperCase(Locale.ENGLISH)
                        .startsWith(columnPrefix)) {
    ...
    private void createRowKeyForMap(ResultSetWrapper rsw, CacheKey cacheKey) throws SQLException {
        ...
        for (String columnName : columnNames) {
            final String value = rsw.getResultSet()
    ...
    private boolean hasTypeHandlerForResultObject(ResultSetWrapper rsw, Class<?> resultType) {
        if (rsw.getColumnNames()
                .size() == 1) {
            return typeHandlerRegistry.hasTypeHandler(
                    resultType, rsw.getJdbcType(rsw.getColumnNames()
                            .get(0)));
        }
