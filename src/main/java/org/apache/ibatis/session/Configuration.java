...
public class Configuration {
    ...
    // TODOHAITAO: 2023/1/11  注：下面用的类型是 StrictMap ，这个的特点是put方法，会校验当前key是否存在了，存在就报错，也就是为了防止重复添加
    ...
    /**
     * incomplete 记录的是未完成解析数据，比如Axml引用了Bxml的信息，但是是Axml先解析的，这就会出现Axml会解析报错，所以
     * 会将解析不了的东西先记录到 incomplete 中
     * */
    ...
    public StatementHandler newStatementHandler(Executor executor, MappedStatement mappedStatement,
        ...
        /**
         * 这是个代理模式，具体干活的是 {@link PreparedStatementHandler}
         * */
        ...
        public V put(String key, V value) {
            if (containsKey(key)) {
                throw new IllegalArgumentException(
                        name + " already contains value for " + key + (conflictMessageProducer == null ? "" : conflictMessageProducer.apply(
                                super.get(key), value)));
            }
            if (key.contains(".")) {
                // 比如 cn.haitaoss.AMapper.list  ---> list
                final String shortKey = getShortName(key);
                // 短名称，没被添加过 说明可以作为 value的唯一标识
                if (super.get(shortKey) == null) {
                    super.put(shortKey, value);
                } else {
                    /**
                     * 如果短名称被重复使用，就不能作为唯一标识，那就把value设置成一个无效的值
                     * todo 这种语法是支持的？？？ 是支持的，我也觉得很神奇，只要不要显示的强制转换就不会报错。
                     * */
